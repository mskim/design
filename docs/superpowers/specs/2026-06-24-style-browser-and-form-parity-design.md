...
| 4 | **Gem-native per-row "Edit" link** (additive; book_design can adopt it). Resolves to the style's level: **override row** → `edit_theme_paper_size_document_design_paragraph_style_path(theme, ps, dd, style)`; **base row** → `edit_theme_theme_paragraph_style_path(theme, style)`. NOTE: the ported `merged_paragraph_styles` pipeline yields only **theme-base + doc-design-override** styles (it does NOT include paper-size-level styles — matching book_design), so a base row's `style.styleable` is **always a `Theme`**; there is no paper-size-level base row, so no `base_paragraph_styles` edit branch is needed. The plan must verify the two helper names with `bin/rails routes` before writing tests. |
...
- **Edit-URL resolution for base rows:** confirmed that `merged_paragraph_styles` returns only theme-base + doc-design-override styles, so base rows are always theme-level (`style.styleable` is a `Theme`). The plan verifies `edit_theme_theme_paragraph_style_path` + `edit_theme_paper_size_document_design_paragraph_style_path` via `bin/rails routes`. If a merged style's `styleable` is unexpectedly nil/other, fall back to no Edit link for that row.
