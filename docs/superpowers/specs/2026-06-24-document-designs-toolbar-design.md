...
| 5 | **Use the gem's i18n doc-type labels** (`doc_type_label`) for the button + menu items, not `titleize`. Promote `doc_type_label` to `Design::Views::Base`, **changing its signature from `dd` to `doc_type` (a string)**, and **update the one existing call site** at `themes/show.rb:133` from `doc_type_label(dd)` → `doc_type_label(dd.doc_type)`. (Confirmed: exactly one call site + one definition; no other callers.) |
